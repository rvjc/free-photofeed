...
function fontMetrics(styleName, elementHeight, fontclass)
{
	...
	// (typeof(styleName) === 'string', 'bad styleName');
	...
	case 'L':
		...
		break;
	default:
		// (false, 'unknown textStyle size', fontclass);
		break;
	}
	...
	default:
		// (false, 'unknown fontclass', fontclass);
		break;
	}
...
function pngPattern(fgcolor, styleName, align, bgcolor)
{
	...
	switch(sc)
	{
		...
		default:
			...
			// (false, 'invalid PngPattern style', s);
	}
	...
	switch(align)
	{
		...
		default:
			// (false, 'invalid PngPattern, align', align);
	}
...
Cfg.prototype.initParam = function(flag, pName, desc, type, def, min, max, minSize, maxSize)
{
	...
	// (maxSize === undefined || maxSize >= 0, "Cfg.initParam: Array Max is Negative", pName);
	// (typeof(type) === 'number' || type instanceof OptList || type instanceof StrList, "Cfg.initParam: Invalid data type", pName);
	// (!((type instanceof OptList || type instanceof StrList) && def === CON.cfg.required), "Cfg.initParam: List parameter must have a default(create a 'NONE' option if necessary)", pName);
	// (typeof(flag) === 'string' && flag.length === 1 && 'VDNFST'.indexOf(flag) >= 0, "Cfg.initParam: Bad flag", pName);
	...
	{	
		if (typeof(type) === 'number')
		{
			...
			case CON.types.color:				  
				...
				break;	  
			default:
				// (false, "Cfg.initParam: Unsupported parameter type", pName);		
			}
			...
			if ( def !== CON.cfg.required )
			{
				...
				{
					// (isArrayOf(def, baseType), "Cfg.initParam: Config default is not an array of the correct type", pName);
				} 
				else
				{
					// (typeof(def) === baseType, "Cfg.initParam: Config default is not the correct type", pName);
				}
			} 
		}
	}
...
{	
	...
	// (this.state === CON.cfgStates.initialising, "Cfg.ProcessTheme: Invalid Config State");
	...
	if (dNode)
	{
		...
		try
		{
			...
			// (tNodes, "Cfg.processTheme: undefined <theme> nodes list");
			...
			// (oNodes, "Cfg.processTheme: undefined <option> nodes list");
			...
			if (oNodesMatch >= 0)
			{
				...
				// (pNodes, "Cfg.processTheme: undefined <p> nodes list");
...
Cfg.prototype.write = function()
{
	...
	// (ENV.view === CON.views.config, "Cfg.write: only allowed in config mode", pName);	  
...
Cfg.prototype.set = function(pName, value)
{
	...
	// (this.params[pName] !== undefined, "Cfg.set: Unknown parameter name", pName);
...
Cfg.prototype.setDefault = function(pName)
{
	...
	p = this.params[pName];	
	// (p !== undefined, "Cfg.setDefault: Unknown parameter name", pName);
...
Cfg.prototype.load = function(pName)
{
	...
	// (ENV.view === CON.views.config, "Cfg.load: Unknown parameter name", pName);
	   
	p = this.params[pName];
	// (p !== undefined, "Unknown config parameter name", pName);	   
	...
	switch(input.type)
	{
		...
		case 'select-one':
			...
			if ( type instanceof OptList || type instanceof StrList )
			{
				...
				// (type.has(val), "Invalid select box value", pName, val);
				input.selectedIndex = type.valueIndex(val);
			}
			else
			{
				// (false, "Select-one box must be bound to OptList or StrList", pName);
			}
			break;
	
		default:
			// (false, "Unsupported Input type", input.type);	  
	}
...
{	
	...
	p = this.params[pName];	   
	// (p !== undefined, "Cfg.loadDefault: Unknown parameter name", pName);	   
...
Cfg.prototype.enable = function(pName, enabled)
{
	...
	p = this.params[pName];	   
	// (p !== undefined, "Cfg.enable: Unknown parameter name", pName);	  
...
Cfg.prototype.notifyParamChange = function(pName, extChangeHandler)
{
	...
	p = this.params[pName];
	// (p !== undefined, "Cfg.notifyParamChange: Unknown parameter name", pName);	   
...
{	
	...
	// (ENV.view === CON.views.config, "Cfg.save: Unknown parameter name");
...
Cfg.prototype.changeHandler = function(event)
{
	...
	p = this.params[pName];
	// (p !== undefined, "Cfg.changeHandler: Unknown parameter name", pName);	   
...
Cfg.prototype.parse = function(pName, raw, index)
{
	...
	// (type & CON.typeMods.array || index === undefined, "Parse index supplied but config parameter is scalar", pName); 
	...
	case CON.types.bool:
		...
		switch(raw.toLowerCase())
		{
			...
			case 'false': case 'no': case '0':
				...
				break;
			default:
				// (false, "Invalid boolean value", pName, raw);
		}
		return val;
	...
	case CON.types.color:		  
		...
		{	  
			...
			rgb = val.split(',');
			// (rgb.length === 3, "RGB regex is incorrect");
	...
	// (false, "Unknown configuration type", pName, type);   	
};
...
Cfg.prototype.toString = function(pName)
{
	...
	p = this.params[pName];   
	// (p !== undefined, "Cfg.toString: Unknown parameter name", pName);
...
Cfg.prototype.toText = function(pName)
{
	...
	p = this.params[pName];   
	// (p !== undefined, "Cfg.toText: Unknown parameter name", pName);
...
function Src()
{
	...
	{
		// (this.type === 'Site', "Source type error"); 
...
{ 
	...
	try
	{
		...
		for (i=0; i < eNodes.length; i++)
		{
			...
			tNodes = eNode.getElementsByTagName("title");
			// (tNodes, "Cab items DOM entry title");
			...
			// (tNodes, "Cab items DOM entry summary");
...
Cab.prototype.preload = function(preloadHandler, offset, length)
{
	...
	// (offset >= 0 && offset < CAB.items.length, "Cab preload offset out of range");
	// (length >= 0 && length <= CAB.items.length, "Cab preload length out of range");
	...
	{		  
		...
		// (this.numPreloading < CON.cab.maxItems, "Cab preload tracking overrun");
		// (item.retries === 0, "Cab item retries should be 0");
...
Cab.prototype.timerHandler = function()
{
	...
	for (i=0; i < this.items.length; i++)
	{
		...
		// (this.numPreloading > 0, "Cab preload tracking underrun");
		...
		if (this.numPreloading <= 0)
		{
			...
			// (this.numPreloading === 0, "Cab numPreloading under-run");			
			// (this.numIdle + this.numPreloaded + this.numAborted === this.numItems(), "Cab tracking problem");
	...
	if (this.preloadHandler)
	{
		...
		for (i = this.notifyOffset; i < (this.notifyOffset + this.notifyLength); i++)	
		{
			...
			// (item.state === CON.imgStates.preloaded || item.state === CON.imgStates.aborted);
		}
...
{ 
	...
	try
	{
		...
		for (i=0; i < eNodes.length; i++)
		{
			...
			// (tNodes, "Cab items DOM entry sites:pageName");
...
{	
	...
	// (gel(divId), "Sel: DivId not found", divId); 
	this.divId = divId;
	...
	// (contentHeight >= CON.content.minHeight, "height validation bug");
...
{	
	...
	// (ac===this.columns.length,"Selection columns tracking error", ac, this.columns.length);
	// (ap > 0, "no available photos", ap);
...
{
	// (this.options.link.length > 0, "expected link");
...
{	
	...
	// (gel(divId), "Sho: DivId not found", divId); 
	this.divId = divId;
	...
	if (this.options.panelHeight > 0)
	{
		...
		else
		{
			...
			// (panelClass === 'pad', 'panel class bug');
		...
		else if (panelClass === 'str')
		{
			...
			switch (this.options.spreadStyle)
			{
				...
				default:
					// (false, 'unknown spread Style', this.options.spreadStyle);
					break;
			}
		}
		else
...
Sho.prototype.mouseOverHandler = function(id)
{
	...
	default:
		...
		// (this.beckonId === null, "beckon over bug");
...
Sho.prototype.mouseOutHandler = function(id)
{
	...
	default:
		...
		// (this.beckonId !== null, "beckon out bug");
...
Sho.prototype.move = function(index)
{
	...
	// (index >= 0 && index <= total, "Bad move index", index);
